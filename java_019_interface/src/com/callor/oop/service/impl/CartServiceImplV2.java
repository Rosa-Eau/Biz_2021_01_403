...
public class CartServiceImplV2 implements CartService {
	/*
	 * 1. cartList와 scan 인스턴스 객체변수를 선언만 하고 생성은 하지 않는다.
	 * 
	 * 이렇게 하면 생성되지 않은 인스턴스 객체 변수의 method를 
	 * 호출하는 코드에서 NullpointerException이 발생할 것이다.
	 * 
	 * 3. NullpointerException을 방지하기 위하여 클래스의 생서자 method에
	 * 객체 변수를 초기화 하는 코드를 반드시 추가해 준다.
	 * 
	 * 즉 인스턴스 변수 선언 -> 생성자에서 객체 변수 초기화(생성)
	 * 

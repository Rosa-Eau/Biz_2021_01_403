 ...
 * 보통 private으로 선언을 한다.
 * VO클래스를 사용하여 선언된 vo객체는 인스턴스 변수에
 * 직접 접근할 수 없다.
 * ScoreVO sVO = new ScoreVO();
 * sVO.strName = "홍길동" --> 사용 불가
 * String name = sVO.strName; --> 사용 불가
 * 
 * private으로 선언된 인스턴스에 접근하기 위한 
 * setter, getter method 들을 통해 접근해야 한다.
 * 
 * 저장할 때 : sVO.setStrName("홍길동");
 * 읽을 때 : name = sVO.getStrName();
 * 
 ...
 * getIntKor() method는 intKor 인스턴스 변수에서 값을 읽기 기능을 한다는 것을 알 수 있다.
 * 
 * 코드 내용이 어떠한지 몰라도 getter와 setter를 통하여 변수에 접근(저장, 읽기)할 수 있다.
 * 
 * 
 */
public class ScoreVO {
	 ...
	 * 값을 저장하도록 설정한다
	 * 
	 * 이때 setIntEng() method는 점수를 매개변수로 전달받아
	 * 유효성 검사를 수행하고 검사가 통과 되었을 때만

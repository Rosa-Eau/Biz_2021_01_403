...
public class Array_01 {
	public static void main(String[] args) {
		...
		// 정수 100을 전달하고 실행하여
		// return 하는 결과를 num변수에 저장하라
		// num 변수에는 0~99까지 중 1개의 숫자가 저장될 것.
		int num = rnd.nextInt(100);
		// num 변수에는 1~100까지 중 1개의 숫자가 저장될 것이다.
		num = rnd.nextInt(100) + 1;

		// 정수 100개를 저장할 배열변수 선언 후 생성(초기화 : 사용할 준비를 하라)
		 ...
		 */
		for (int i = 0; i < intNums.length; i++) {
			//Random 클래스의 nextInt() method를 호출하면서
			//정수 10을 전달하고 return된 결과에 1을 더하라
		...
		System.out.print("1 ~ 10 까지의 정수 입력 >> ");
		int intScan = scan.nextInt();
		int intCount = 0;
		for (int i = 0; i < intNums.length; i++) {
			if (intNums[i] == intScan) {
				intCount += 1;
			}
		}

		System.out.printf("%d(은)는 배열 intNums에 %d개 저장되어 있습니다.", intScan, intCount);
